...
`).toLowerCase();
    ...
    const appendCards = (title, entries) => {
      ...
      for (const entry of entries) {
        ...
        owner.textContent = primary ? `${entry.definitions.length > 1 ? `${entry.definitions.length} definitions` : primary.blockName} · ${entry.references.reduce((sum, ref) => sum + ref.count, 0)} refs` : "Missing definition";
  ...
  const editor = ctx.components.mountLoomBlockEditor(editorMount, {
  ...
  root.querySelector('[data-action="left"]').addEventListener("click", () => root.classList.toggle("left-collapsed"));
  root.querySelector('[data-action="right"]').addEventListener("click", () => root.classList.toggle("right-collapsed"));
